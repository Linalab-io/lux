# Lux Rust Gateway Prototype

Phase 1 Rust/WebSocket gateway prototype for `com.linalab.lux`.

This is intentionally a tilde-suffixed external tool project, matching the
existing `Cli~`, `TypeScriptServer~`, and `McpHelper~` convention. It is not a
production replacement for every possible Unity automation command yet.
...
For local package-cache paths, replace `Packages/com.linalab.lux/RustGateway~`
with the actual path to this `RustGateway~` directory.
